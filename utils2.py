...
def load_data_houston(image_file, label_file, is_norm=False):
...
def load_data_houston2(source_image_file, source_label_file, target_image_file, target_label_file,
                       is_pca=False, num_components=30):
    source_image = sio.loadmat(source_image_file)['ori_data']
    source_label = sio.loadmat(source_label_file)['map']
    target_image = sio.loadmat(target_image_file)['ori_data']
    target_label = sio.loadmat(target_label_file)['map']
    if is_pca:
        print('== PCA tranformation ==')
        h_s, w_s = source_image.shape[0], source_image.shape[1]
        h_t, w_t = target_image.shape[0], target_image.shape[1]
        X_s = np.reshape(source_image, (-1, source_image.shape[2]))
        bn = X_s.shape[0]
        X_t = np.reshape(target_image, (-1, target_image.shape[2]))
        X = np.concatenate([X_s, X_t], axis=0)
        # pca = PCA(n_components=num_components, whiten=True)
        pca = PCA(n_components=num_components, whiten=False)
        X = pca.fit_transform(X)
        print(np.max(X), np.min(X))
        # X_scaler = preprocessing.minmax_scale(X)  # 标准化 (X-X_mean)/X_std,
        # print(np.max(X_scaler), np.min(X_scaler))
        # source_image = np.reshape(X_scaler[:bn, :], (h_s, w_s, num_components))
        # target_image = np.reshape(X_scaler[bn:, :], (h_t, w_t, num_components))
        #########
        X_s = X[:bn, :]
        X_t = X[bn:, :]
        X_s_scaler = preprocessing.minmax_scale(X_s)  # 归一化到0和1之间,
        X_t_scaler = preprocessing.minmax_scale(X_t)  # 归一化
        source_image = np.reshape(X_s_scaler, (h_s, w_s, num_components))
        target_image = np.reshape(X_t_scaler, (h_t, w_t, num_components))
